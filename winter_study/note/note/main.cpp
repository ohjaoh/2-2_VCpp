	...
	case WM_COMMAND:
		...
		case ID_SAVE_BUTTON: {  // "저장하기" 버튼의 ID
			...
			if (GetSaveFileName(&ofn)) {
				std::wofstream file(szFileName, std::ios::binary); // 바이너리 모드로 파일 열기
				file.imbue(std::locale(file.getloc(), new std::codecvt_utf16<wchar_t, 0x10ffff, std::little_endian>())); // UTF-16 인코딩 설정

				if (file.is_open()) {
					file.write(textBuffer, textLength); // UTF-16으로 텍스트 쓰기
					file.close();
				}
			}
		}
		...
		case ID_LOAD_BUTTON: {  // "불러오기" 버튼의 ID
			...
			if (GetOpenFileName(&ofn)) {
				std::wifstream file(szFileName, std::ios::binary); // 바이너리 모드로 파일 열기
				file.imbue(std::locale(file.getloc(), new std::codecvt_utf16<wchar_t, 0x10ffff, std::little_endian>())); // UTF-16 인코딩 설정

				if (file.is_open()) {
					textLength = 0; // 버퍼 초기화

					// 파일 내용을 textBuffer에 읽어들임
					wchar_t wc;
					while (file.read(&wc, 1)) {
						if (textLength < sizeof(textBuffer) / sizeof(wchar_t) - 1) {
							textBuffer[textLength++] = wc;
						}
						else {
							break; // 버퍼 오버플로우 방지
						}
					}
					textBuffer[textLength] = L'\0'; // 널 문자로 문자열 종료
